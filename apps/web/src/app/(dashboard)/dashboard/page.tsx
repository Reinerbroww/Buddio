    ...
    <div className="max-w-4xl mx-auto py-6 sm:py-8 space-y-8 animate-in fade-in duration-300">
      ...
      {usage && (
        <div className="bg-gradient-to-r from-[#4F8EF7]/8 to-[#7C5CFF]/8 border border-slate-100 rounded-2xl p-4 flex flex-wrap items-center gap-x-6 gap-y-2">
          <span className="text-[11px] font-extrabold text-slate-500 uppercase tracking-wider">Kuota AI hari ini</span>
          <span className="text-xs text-slate-600">💬 Chat <b>{usage.chat_remaining}/{usage.limits.chat}</b></span>
          <span className="text-xs text-slate-600">🗺️ Roadmap <b>{usage.roadmap_remaining}/{usage.limits.roadmap}</b></span>
          <span className="text-xs text-slate-600">📝 Kuis <b>{usage.quiz_remaining}/{usage.limits.quiz}</b></span>
        </div>
      )}
      ...
      {topics.length > 0 ? (
        <div className="space-y-8 animate-in fade-in duration-300">
          ...
          <div className="space-y-4">
            <h3 className="text-xs font-extrabold text-slate-400 uppercase tracking-wider">Lanjutkan Belajar</h3>
            <div className="bg-white border border-slate-100 rounded-2xl p-6 shadow-xs hover:shadow-md transition-all duration-300 relative overflow-hidden group">
              ...
              <div className="space-y-5 relative z-10">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  ...
                  <div className="flex items-center justify-between sm:justify-end gap-6">
                    <span className="text-lg font-extrabold text-slate-900">{active?.progress_percentage ?? 0}%</span>
                    <Link
          ...
          {others.length > 0 && (
            <div className="space-y-4">
              <h3 className="text-xs font-extrabold text-slate-400 uppercase tracking-wider">Topik Aktif Lainnya</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {others.map((topic) => (
                  <Link
                    key={topic.id}
                    href={`/dashboard/roadmap?topic=${topic.id}`}
                    className="bg-white border border-slate-100 rounded-xl p-5 hover:border-[#4F8EF7]/30 transition-all duration-200 flex flex-col justify-between gap-4"
                  >
                    <div>
                    ...
                    <div className="space-y-1.5">
                      <div className="flex justify-between items-center text-[10px] font-semibold text-slate-400">
                        <span>Progres</span>
                        <span className="text-slate-800">{topic.progress_percentage}%</span>
                      </div>
          ...
          <div className="space-y-4 pt-4 border-t border-slate-100">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-xs font-extrabold text-slate-400 uppercase tracking-wider">Rekomendasi Belajar AI</h3>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {aiRecommendations.map((rec) => (
                <div key={rec.name} className="bg-white border border-slate-100 rounded-xl p-5 hover:border-[#7C5CFF]/30 transition-all duration-200 flex flex-col justify-between gap-4 group">
        ...
        <div className="flex flex-col items-center justify-center text-center py-20 px-4 space-y-6 animate-in fade-in duration-300">
          ...
          <div className="space-y-2 max-w-sm">
            <h3 className="text-base font-bold text-slate-900">Belum ada topik aktif</h3>
          ...
          <button
            onClick={() => setShowModal(true)}
            className="inline-flex items-center gap-2 px-5 py-2.5 bg-slate-50 hover:bg-slate-100 text-xs font-bold text-[#4F8EF7] rounded-xl transition-all duration-300 group border border-slate-100"
          >
      ...
      {showModal && (
        <>
          <div className="fixed inset-0 bg-slate-950/40 backdrop-blur-xs z-50 animate-in fade-in duration-300" onClick={() => setShowModal(false)} />
          <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-2xl shadow-xl border border-slate-100 p-6 w-[90%] max-w-md z-50 animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between pb-4 border-b border-slate-100">
              <h3 className="font-bold text-slate-900 text-base flex items-center gap-2">
                <Sparkles className="w-5 h-5 text-[#4F8EF7]" />
                Topik Belajar Baru
              </h3>
              <button onClick={() => setShowModal(false)} className="p-1 rounded-lg text-slate-400 hover:bg-slate-50 hover:text-slate-600 transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            <form onSubmit={handleCreateTopic} className="mt-4 space-y-4">
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Apa yang ingin kamu pelajari?</label>
                <input
                  type="text"
                  required
                  value={topicName}
