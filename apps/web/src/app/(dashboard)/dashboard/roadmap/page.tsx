...
function difficultyStyle(difficulty: string | null | undefined) {
  ...
  return { label: difficulty, className: "bg-slate-50 dark:bg-[#0f172a] text-slate-600 dark:text-slate-400 border-slate-100 dark:border-[#334155]" };
}
      ...
      {selectedTopicId == null ? (
          ...
          {topicsLoading ? (
            ...
            <div className="space-y-3">
              <select
                ...
                className="w-full px-4 py-3 text-sm bg-slate-50 dark:bg-[#0f172a] border border-slate-100 dark:border-[#334155] focus:border-[#4F8EF7] focus:bg-white dark:focus:bg-[#0f172a] rounded-xl outline-none transition-all text-slate-900 dark:text-slate-100"
              >
              ...
              <div className="space-y-2">
                {topics.map((item) => (
                  <button
                    ...
                    className="w-full flex items-center justify-between gap-3 px-4 py-3 bg-slate-50 dark:bg-[#0f172a] hover:bg-slate-100 dark:hover:bg-[#334155] border border-slate-100 dark:border-[#334155] rounded-xl transition-all duration-200 text-left group"
                  >
        ...
        <div className="bg-white dark:bg-[#1e293b] border border-slate-100 dark:border-[#334155] rounded-2xl p-8 sm:p-12 flex flex-col items-center text-center gap-6">
          ...
          <div className="space-y-2 max-w-md">
            <h3 className="text-base font-bold text-slate-900 dark:text-slate-100">{t("roadmap.noRoadmapTitle")}</h3>
        ...
        <>
          <div className="bg-white dark:bg-[#1e293b] border border-slate-100 dark:border-[#334155] rounded-2xl p-6 sm:p-8 space-y-6 relative overflow-hidden">
            ...
            <div className="relative z-10 space-y-5">
              ...
              <div>
                ...
                {topic && <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">{t("roadmap.topicLabel", { topic: topic.title })}</p>}
              </div>
              ...
              <div>
                <div className="flex justify-between items-center text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1.5">
                  ...
                  <span className="text-slate-900 dark:text-slate-100 font-extrabold">{roadmap.completion_percentage}%</span>
                </div>
          ...
          <div className="space-y-3">
            <h3 className="text-xs font-extrabold text-slate-400 dark:text-slate-500 uppercase tracking-wider">{t("roadmap.learningSteps")}</h3>
            {[...roadmap.steps]
                  ...
                  <div key={step.id} className="bg-white dark:bg-[#1e293b] border border-slate-100 dark:border-[#334155] rounded-2xl overflow-hidden">
                    <div className="flex items-center gap-3 p-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase">{t("roadmap.step", { n: step.order_number })}</p>
                        <p className={`text-sm font-semibold ${step.completed ? "text-slate-400 dark:text-slate-500 line-through" : "text-slate-900 dark:text-slate-100"}`}>
                          {step.title}
                        </p>
                      </div>
                      {step.lesson_id && (
                      ...
                      <button
                        ...
                        className="shrink-0 p-1 rounded-lg text-slate-400 dark:text-slate-500 hover:bg-slate-50 dark:hover:bg-[#334155] hover:text-slate-600 dark:hover:text-slate-300 transition-colors"
                      >
                        <ChevronDown
                    ...
                    {isExpanded && (
                      <div className="px-4 pb-4">
                        <div className="pl-9">
                          {step.description ? (
                            <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed whitespace-pre-wrap">{step.description}</p>
                          ) : (
                            <p className="text-xs text-slate-400 dark:text-slate-500 italic">{t("roadmap.noDesc")}</p>
                          )}
                        </div>
                      </div>
                    )}
      ...
      {generating && (
        <>
          ...
          <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white dark:bg-[#1e293b] rounded-2xl shadow-xl border border-slate-100 dark:border-[#334155] p-6 w-[90%] max-w-sm z-50 animate-in fade-in zoom-in-95 duration-200">
            ...
            <p className="text-right text-xs font-bold text-slate-500 dark:text-slate-400 mt-1.5">{progress}%</p>
          </div>
        </>
      )}
